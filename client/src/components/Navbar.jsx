...
const Navbar = () => {
    ...
    return(
        <nav className="flex items-center justify-between gap-3 px-4 sm:px-6 md:px-16 lg:px-24 xl:px-32 py-3.5 sm:py-4 border-b border-gray-300 bg-white relative transition-all">

            <NavLink to='/' onClick={() => setOpen(false)} className="shrink-0">
                <img className="h-8 sm:h-9 md:h-10 w-auto object-contain" src={assets.logo} alt="Logo" />
            </NavLink>
            {/* Desktop Menu */}
            <div className="hidden sm:flex items-center gap-5 md:gap-8">
                ...
                <div className="hidden lg:flex items-center text-sm gap-2 border border-gray-300 px-3 rounded-full min-w-56 xl:min-w-80">
            ...
            {/* Mobile Menu */}
            {open && (
            <div className={`${open ? 'flex' : 'hidden'} absolute top-full left-0 w-full bg-white shadow-md py-4 flex-col items-start gap-2 px-5 text-sm md:hidden z-30`}>
